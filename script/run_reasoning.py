...
from config import BEGIN_SEARCH_QUERY, END_SEARCH_QUERY, BEGIN_CODE_QUERY, END_CODE_QUERY, BEGIN_MIND_MAP_QUERY, END_MIND_MAP_QUERY, BEGIN_SEARCH_RESULT, END_SEARCH_RESULT, BEGIN_CODE_RESULT, END_CODE_RESULT, BEGIN_MIND_MAP_RESULT, END_MIND_MAP_RESULT
import litellm
import os
import litellm
...
def run_reasoning(filtered_data: list, active_sequences: list, forcing_research: bool , general_model_args: dict, code_mode_name: str = "gpt-4o", tokenizer: object = None, tavily_client: object = None, verbose: bool = False) -> list:
    """
    ...
    for i in range(len(filtered_data)):
        ...
        code_agent = CodeAgent(model_name=code_mode_name, api_key=os.environ["OPENAI_API_KEY"])
        ...
        while not sequence["finished"]:
            ...
            research_query = extract_between(output, BEGIN_SEARCH_QUERY, END_SEARCH_QUERY)
            ...
            if research_query is None and code_query is None and mind_map_query is None:
                ...
                if sequence["final_result"] is not None and sequence["final_result"] not in ['a','b','c','d']:
                    print(f"Final result is not in the correct format: {sequence['final_result']}")
