...
def get_instruction_and_prompt(
    ...
    if dataset_name in ["nq", "triviaqa", "hotpotqa", "musique", "bamboogle", "2wiki"]:
        ...
        user_prompt = get_task_instruction_openqa(
            question, model_name="qwq" if is_qwq else None
        )

    elif dataset_name in ["math500", "aime", "amc"]:
        instruction = get_math_search_o1_instruction(max_search_limit)
        user_prompt = get_task_instruction_math(
            question, model_name="qwq" if is_qwq else None
        )
...
def prepare_input_prompts(
    ...
    for item in filtered_data:
        ...
        question_title = (
            item.get("question_title", "") if dataset_name == "livecode" else None
        )

        instruction, user_prompt = get_instruction_and_prompt(
            dataset_name, question, model_path, max_search_limit, question_title
        )
